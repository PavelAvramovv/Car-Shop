...
int main()
{
	...
	int inputDoors;
	std::cout << "Enter the number of car doors (4 or 2): ";
	std::cin >> inputDoors;
	std::cout << std::endl;
	...
	if (inputDoors == 4)
	{
		for (int i = 1; i < AllCars.size(); i++)
		{
			...
			if (AllCars[i].nDoors == 4)//Може да се смени условето за избор на брой врати.
			{
		...
		for (int i = 1; i < AllCars.size(); i++)
		{
			if (AllCars[i].nKilometers < 100000 && AllCars[0].nPrice < AllCars[i].nPrice)//Може да се смени условето за избор на пробег и цена.
	...
	///////////////////////////////////////////
	else 	if (inputDoors == 2)
	{
		for (int i = 1; i < AllCars.size(); i++)
		{

			if (AllCars[i].nDoors == 2)//Може да се смени условето за избор на брой врати.
			{
				std::cout << "Car Mark: " << AllCars[i].strMark << std::endl;
				std::cout << "Car color: " << AllCars[i].strcolor << std::endl;
				std::cout << "Car Doors: " << AllCars[i].nDoors << std::endl;
		...
		std::cout << "Invalid number, please try again and enter 2 or 4 ! ";
	}
}